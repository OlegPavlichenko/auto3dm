...
function SafeModelViewer({ src, alt, poster }: { src: string; alt: string; poster?: string }) {
  ...
  const [status, setStatus] = useState<'loading' | 'ok' | 'error' | 'unsupported' | 'notready'>('loading');
  ...
  useEffect(() => {
    if (!ext) return; // unknown — let viewer try later
    const ok = ext === 'glb' || ext === 'gltf'; // <model-viewer> supports glTF 2.0; prefer .glb
    if (!ok) setStatus('unsupported');
  }, [ext]);

  // Handle readiness and hook events only when custom element is defined
  useEffect(() => {
    if (!mvReady) {
      setStatus((s) => (s === 'unsupported' ? s : 'notready'));
      return;
    }
    ...
    // When src changes and we're ready, transition to loading state
  ...
  }, [normalizedSrc, mvReady, status]);

  // Preferred UX (confirmed by user): if error/unsupported → hide 3D viewer and show poster only
  const showPosterOnly = status === 'error' || status === 'unsupported' || status === 'notready';

  return (
    <div className="relative w-full h-full">
      {(status === 'loading' && mvReady) && (
      ...
      {/* render viewer only when element is registered and not in poster-only state */}
      {mvReady && !showPosterOnly && (
